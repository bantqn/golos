...
## Установочный образ без Developer ID

```bash
./build.sh dmg
```

Команда создаёт `dist/golos-1.0-macos-arm64.dmg` и файл контрольной суммы.
Внутри находятся приложение, ярлык «Программы» и короткая инструкция. Из-за
ad-hoc подписи первый запуск выполняется через правую кнопку → «Открыть».

## Публичный релиз с Developer ID
