...
1. [скачайте `golos-1.0-macos-arm64.dmg`](https://github.com/bantqn/golos/releases/latest/download/golos-1.0-macos-arm64.dmg);
2. откройте скачанный образ;
3. перетащите «голос» на ярлык «Программы»;
4. при первом запуске нажмите на приложение правой кнопкой и выберите
   **Открыть → Открыть**. это одноразовое подтверждение macOS для сборки без
   сертификата Apple Developer ID.

Если кнопки запуска нет, откройте **Системные настройки → Конфиденциальность
и безопасность → Всё равно открыть**. Терминал не требуется.

Готовый установщик предназначен для Mac с Apple Silicon и macOS 14 или новее.
...
./build.sh dmg      # собрать установочный dmg
